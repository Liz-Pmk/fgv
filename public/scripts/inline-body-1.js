(() => {
    const initializeExperience = () => {
        ...
        if (infoPopover && popoverTitle && popoverContent) {
            const positionPopover = (event, trigger) => {
                const touch = event.touches && event.touches[0];
                const rect = trigger.getBoundingClientRect();
                let left = touch ? touch.clientX + 15 : event.clientX + 15;
                let top = touch ? touch.clientY + 15 : event.clientY + 15;
                ...
                if (left + popoverWidth > window.innerWidth - 15) {
                    left = (touch ? touch.clientX : rect.right) - popoverWidth - 15;
                }
                if (top + popoverHeight > window.innerHeight - 15) {
                    top = (touch ? touch.clientY : rect.top) - popoverHeight - 15;
                }
            ...
            const showPopover = (event) => {
                ...
                const title = trigger.dataset.title || trigger.dataset.fundamento;
                ...
                if (title && content) {
                    ...
                    positionPopover(event, trigger);
                    activeTrigger = trigger;
                }
            };

            const movePopover = (event) => {
                if (!infoPopover.classList.contains('opacity-0') && activeTrigger) {
                    positionPopover(event, activeTrigger);
                }
            };
            ...
            document.addEventListener('click', (event) => {
                if (activeTrigger && !activeTrigger.contains(event.target) && !infoPopover.contains(event.target)) {
                    hidePopover();
                }
            });
            ...
            document.querySelectorAll('.neuro-insight, .rich-nav-icon').forEach(trigger => {
                ...
                trigger.addEventListener('mousemove', movePopover);
